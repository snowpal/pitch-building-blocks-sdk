...
import (
	...
	collaboration "development/go/recipes/lib/building-blocks/endpoints/collaboration.1.blocks"
	keyPods "development/go/recipes/lib/building-blocks/endpoints/key_pods.1"
...
func ValidateDependencies() (response.User, error) {
	user, err := SignIn(building_blocks.DefaultEmail, building_blocks.Password)
	fmt.Println(user)
	fmt.Println(err)
	if err != nil {
		return user, err
...
func AddCustomKey(user response.User, keyName string) (response.Key, error) {
	newKey, err := addKey(user, keyName, building_blocks.CustomKeyType)
	if err != nil {
		return newKey, err
	}
	return newKey, nil
...
func AddTeacherKey(user response.User, keyName string) (response.Key, error) {
	newKey, err := addKey(user, keyName, building_blocks.TeacherKeyType)
	if err != nil {
		return newKey, err
	}
	return newKey, nil
...
func AddProjectKey(user response.User, keyName string) (response.Key, error) {
	newKey, err := addKey(user, keyName, building_blocks.ProjectKeyType)
	if err != nil {
		return newKey, err
	}
	return newKey, nil
