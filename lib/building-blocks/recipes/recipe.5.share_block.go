...
import (
	...
	user2 "development/go/recipes/lib/building-blocks/endpoints/user.1"
...
func getWriteUser(user response.User, block response.Block) (response.User, error) {
	...
	writeUser, err = recipes.SignIn(writeUser.Email, building_blocks.Password)
	if err != nil {
		return writeUser, err
	}
...
func makeReadUserAsAdmin(user response.User, block response.Block) error {
	...
	for _, sharedUser := range *resBlock.SharedUsers {
		if sharedUser.Acl == building_blocks.ReadAcl {
			readUser = sharedUser
			break
		}
	}
	...
	_, err = collaboration.UpdateBlockAcl(
		user.JwtToken,
		request.BlockAclReqBody{Acl: building_blocks.AdminAcl},
		common.AclParam{
			UserId: readUser.ID,
